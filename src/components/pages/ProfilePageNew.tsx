            ...
            <div className="space-y-6">
              ...
              <Card className="rounded-2xl border-slate-200">
                ...
                <CardContent className="grid grid-cols-3 gap-3">
                  {[
                    { icon: Trophy, color: "text-amber-500 bg-amber-100", unlocked: true, name: "Первые шаги" },
                    { icon: Star, color: "text-blue-500 bg-blue-100", unlocked: true, name: "Знаток" },
                    { icon: Award, color: "text-purple-500 bg-purple-100", unlocked: true, name: "Мастер" },
                    { icon: Crown, color: "text-amber-400 bg-amber-50", unlocked: false, name: "Чемпион" },
                    { icon: Shield, color: "text-slate-300 bg-slate-100", unlocked: false, name: "Защитник" },
                    { icon: Heart, color: "text-slate-300 bg-slate-100", unlocked: false, name: "Любимец" }
                  ].map((achievement, i) => {
                    const Icon = achievement.icon;
                    return (
                      <div 
                        key={i} 
                        className={`aspect-square rounded-xl flex items-center justify-center hover:scale-105 transition-all cursor-help group relative ${achievement.color} ${!achievement.unlocked && 'opacity-40'}`}
                      >
                        <Icon className="h-6 w-6" />
                        <div className="absolute bottom-full mb-2 hidden group-hover:block bg-slate-900 text-white text-xs p-2 rounded-lg whitespace-nowrap z-10">
                          {achievement.name}
                        </div>
                      </div>
                    );
              ...
              <Card className="rounded-2xl border-slate-200">
                <CardHeader>
                  ...
                  <CardDescription>Ваша позиция: #127</CardDescription>
                </CardHeader>
                <CardContent className="space-y-3">
                  {[
                    { name: "Мария К.", score: 2840, position: 1 },
                    { name: "Дмитрий Н.", score: 2720, position: 2 },
                    { name: "Елена С.", score: 2690, position: 3 },
                    { name: "Вы", score: 1450, position: 127 }
                  ].map((user, i) => (
                    <div key={i} className={`flex items-center gap-3 p-2 rounded-lg ${user.position === 127 ? 'bg-blue-50' : ''}`}>
                      <div className={`h-6 w-6 rounded-full flex items-center justify-center text-xs ${
                        user.position === 1 ? 'bg-amber-400 text-white' :
                        user.position === 2 ? 'bg-slate-300 text-white' :
                      ...
                      <span className="text-xs text-muted-foreground">{user.score} XP</span>
                    </div>
                  ))}
                </CardContent>
              </Card>
